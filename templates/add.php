 ...
 * @var array $form_data массив с отфильтрованными данными из формы
 */
?>
    ...
    <form class="form form--add-lot container <?= !empty($error) ? 'form--invalid' : '' ?>" action="add.php" method="post"  enctype="multipart/form-data">
            ...
            <div class="form__item <?= isset($error['category_id']) ? 'form__item--invalid' : '' ?>">
                ...
                <select id="category" name="category_id">
                    ...
                    <?php foreach ($categories as $category) : ?>
                        <option
                            value="<?= $category['id'] ?>" <?= get_post_select($form_data, $category['id']) ?>><?= $category['name'] ?></option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="form__item form__item--wide <?= isset($error['description']) ? 'form__item--invalid' : '' ?>">
            ...
            <textarea id="description" name="description"
                      placeholder="Напишите описание лота"><?= $form_data['description'] ?? '' ?></textarea>
        ...
        <div class="form__container-three">
            <div class="form__item form__item--small <?= isset($error['price']) ? 'form__item--invalid' : '' ?>">
                ...
                <input id="price" type="text" name="price" placeholder="0" value="<?= $form_data['price'] ?? '' ?>">
            ...
            <div class="form__item form__item--small <?= isset($error['step']) ? 'form__item--invalid' : '' ?>">
                ...
                <input id="step" type="text" name="step" placeholder="0" value="<?= $form_data['step'] ?? '' ?>">
            ...
            <div class="form__item <?= isset($error['dt_end']) ? 'form__item--invalid' : '' ?>">
                ...
                <input class="form__input-date" id="lot-date" type="text" name="dt_end"
                       placeholder="Введите дату в формате ГГГГ-ММ-ДД" value="<?= $form_data['dt_end'] ?? '' ?>">
